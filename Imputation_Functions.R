...
Impute_and_VI <- function(data, ntreesimp=300, ntrees=500, nmult=5, ntechs=6, xvar){
  ...
  rfMiss <- randomForest(y~., data=Imputed, ntree=ntrees,importance=TRUE)
  #store variable importance resulting from RFimpute in first column  
  if(is.factor(y)){VI[,1]<-rfMiss$importance[,3]}else{VI[,1]<-rfMiss$importance[,1]} #If y is a factor, permutation importance is given in 3rd col. Otherwise first
  ...
  rfMiss <- randomForest(y~., data=Imputed, ntree=ntrees,importance=TRUE)
  if(is.factor(y)){VI[,2]<-rfMiss$importance[,3]}else{VI[,2]<-rfMiss$importance[,1]} #If y is a factor, permutation importance is given in 3rd col. Otherwise first
  
  #Impute using RFSRC-1 iteration
  rfMiss <- rfsrc(y~., data=data, ntree=ntrees,importance=c("permute"), na.action=c("na.impute"), nimpute=1)
  if(is.factor(y)){VI[,3]<-rfMiss$importance[,1]}else{VI[,3]<-rfMiss$importance} #If y is a factor, permutation importance is given in 1st col. Otherwise just a vector
  ...
  if(is.factor(y)){VI[,4]<-rfMiss$importance[,1]}else{VI[,4]<-rfMiss$importance} #If y is a factor, permutation importance is given in 1st col. Otherwise just a vector
...
#function to generated data, then delete and impute for all variables of interest.
Del_Impute_wrapper <- function(data, xvarvec, pvec, ntrees=500){
  VI <- lapply(X=xvarvec, pvec=pvec, data=data, FUN=Del_Impute)
  return(VI)
}
